...
`[label](https://url)` in markdown. The `mdx-components.tsx` override gives it `text-hyper hover:underline` and `target="_blank" rel="noopener noreferrer"` automatically. Don't write raw `<a>` inside markdown.

### Add a new experience

Copy any `<Section>` block in `page.mdx` and edit the props + markdown body. Required: `company`. Optional: `role`, `period`, `links`. The title is composed by `Section` as `> {company} - [ {role}, {period} ]`, with `role` and `period` dropped individually when empty. If `links` is omitted or empty, the `[ ]` bracket is hidden (this is why the `crystalize` entry has no bracket).
...
`<About>` is the `> me` block (heading is hardcoded; only the body changes). `<Header>` takes `name` and a `socials` array of `[label, href]` tuples. Header social links use `text-foreground/60` (not `text-hyper`) and intentionally do NOT open in new tabs — that's existing behavior, not a bug.
...
- `text-hyper` and `border-hyper` use `--hyper`, a CSS variable that's mutated at runtime by the `r` key (cycled through `FAVORITE_COLORS` in `theme-provider.tsx`) and persisted to `localStorage`. To add a color, append a hex to that array.
- `text-foreground/60` is the muted body color; `text-foreground/30` is the theme-hint color. Tokens come from Tailwind v4 `@theme inline { … }` in `globals.css`.
...
`@import url(...)` (e.g. the Geom Google Font) must appear **before** `@import "tailwindcss"` in `globals.css`. Tailwind v4 expands its import inline at build time, and Turbopack rejects `@import url(...)` that follows expanded rules ("@import rules must precede all rules…"). The Geom import is currently at line 1 for this reason — don't move it.

## Theme + keys

`t` toggles `dark` class on `<html>`. `r` cycles `--hyper`. Both persist via `localStorage` and are restored by an inline script in `layout.tsx` that runs before React hydrates (avoids flash). Don't move the theme initialization into `ThemeProvider` — the flash-prevention depends on it being inline in `<head>`.
